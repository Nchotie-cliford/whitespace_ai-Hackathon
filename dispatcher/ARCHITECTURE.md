...
For the hackathon MVP, we are building a **Pure LLM Dispatcher**. This approach leverages a Large Language Model (e.g., OpenAI's `gpt-4o`) to act as the primary reasoning engine for scheduling and task assignment. 
...
2. **Processing:** We serialize this JSON and pass it to the LLM via the `openai` API, utilizing **Structured Outputs** to force the LLM to reply firmly with a valid JSON array.
